...
function plainScore(score) {
  if (score >= 90) return { color: '#8D7CFF', title: 'Votre site est très bien protégé', sub: 'Continuez comme ça !' };
  if (score >= 75) return { color: '#8D7CFF', title: 'Votre site est plutôt bien protégé', sub: 'Quelques points peuvent encore être améliorés.' };
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .brand .b{background:linear-gradient(120deg,#8D7CFF,#8D7CFF);-webkit-background-clip:text;background-clip:text;color:transparent}
  ...
  .pill.ok{background:rgba(167,243,107,.14);color:#8D7CFF}.pill.warn{background:rgba(255,151,21,.16);color:#FF9715}.pill.unk{background:rgba(255,255,255,.06);color:#8B98A8}
  ...
  .pitem-r{font-size:13.5px;color:#c8e6a6;background:rgba(167,243,107,.1);border-left:3px solid #8D7CFF;margin-top:8px;padding:8px 11px;border-radius:0 8px 8px 0}
  ...
  .fbadge.ok{background:rgba(167,243,107,.14);color:#8D7CFF}.fbadge.bad{background:rgba(244,87,107,.16);color:#F4576B}
  .allgood{background:rgba(167,243,107,.08);border:1px solid rgba(167,243,107,.25);color:#8D7CFF;border-radius:14px;padding:16px;font-weight:600}
  ...
  .ok{color:#8D7CFF;font-size:14px}
...
<body><div class="page">
  <header class="cover">
    <div class="brand"><span class="blogo">${brand.picto ? brand.picto('radar', '#8b6cff') : ''}</span> <span>Sentinel<span class="b">Scope</span></span></div>
